...
static const char g_dpsIdScopeEnvironmentVariable[] = CONFIG_DPS_ID_SCOPE;

// Environment variable used to specify this application's DPS device id
static const char g_dpsDeviceIdEnvironmentVariable[] = CONFIG_DEVICE_REGISTRATION_ID;

// Environment variable used to specify this application's DPS device key
static const char g_dpsDeviceKeyEnvironmentVariable[] = CONFIG_DPS_SYMMETRIC_KEY;
...
static const char g_temperatureControllerModelId[] = "dtmi:Espressif:SensorController;2";
...
PNP_TELEMETRIES_COMPONENT_HANDLE g_thermostatHandle1;
PNP_TELEMETRIES_COMPONENT_HANDLE g_thermostatHandle3;
PNP_TELEMETRIES_COMPONENT_HANDLE g_thermostatHandle4;
PNP_TELEMETRIES_COMPONENT_HANDLE g_thermostatHandle5;
PNP_TELEMETRIES_COMPONENT_HANDLE g_thermostatHandle6;
PNP_TELEMETRIES_COMPONENT_HANDLE g_thermostatHandle7;
PNP_TELEMETRIES_COMPONENT_HANDLE g_thermostatHandle8;
PNP_TELEMETRIES_COMPONENT_HANDLE g_thermostatHandle9;
PNP_TELEMETRIES_COMPONENT_HANDLE g_thermostatHandle10;
PNP_THERMOSTAT_COMPONENT_HANDLE g_thermostatHandle11;

// Name of subcomponents that TemmperatureController implements.
static const char g_thermostatComponent1Name[] = "SensorAltitude";
static const size_t g_thermostatComponent1Size = sizeof(g_thermostatComponent1Name) - 1;
static const char g_thermostatComponent3Name[] = "SensorHumid";
static const size_t g_thermostatComponent3Size = sizeof(g_thermostatComponent3Name) - 1;
static const char g_thermostatComponent4Name[] = "SensorLight";
static const size_t g_thermostatComponent4Size = sizeof(g_thermostatComponent4Name) - 1;
static const char g_thermostatComponent5Name[] = "SensorMagnetX";
static const size_t g_thermostatComponent5Size = sizeof(g_thermostatComponent5Name) - 1;
static const char g_thermostatComponent6Name[] = "SensorMagnetY";
static const size_t g_thermostatComponent6Size = sizeof(g_thermostatComponent6Name) - 1;
static const char g_thermostatComponent7Name[] = "SensorMagnetZ";
static const size_t g_thermostatComponent7Size = sizeof(g_thermostatComponent7Name) - 1;
static const char g_thermostatComponent8Name[] = "SensorPitch";
static const size_t g_thermostatComponent8Size = sizeof(g_thermostatComponent8Name) - 1;
static const char g_thermostatComponent9Name[] = "SensorPressure";
static const size_t g_thermostatComponent9Size = sizeof(g_thermostatComponent9Name) - 1;
static const char g_thermostatComponent10Name[] = "SensorRoll";
static const size_t g_thermostatComponent10Size = sizeof(g_thermostatComponent10Name) - 1;
static const char g_thermostatComponent11Name[] = "SensorTemp";
static const size_t g_thermostatComponent11Size = sizeof(g_thermostatComponent11Name) - 1;
static const char g_deviceInfoComponentName[] = "deviceInformation";

static const char* g_modeledComponents[] = {g_thermostatComponent1Name, g_thermostatComponent3Name, 
                                            g_thermostatComponent4Name, g_thermostatComponent5Name, g_thermostatComponent6Name,
                                            g_thermostatComponent7Name, g_thermostatComponent8Name, g_thermostatComponent9Name,
                                            g_thermostatComponent10Name, g_thermostatComponent11Name,
                                            g_deviceInfoComponentName};
    ...
    *responseSize = 0;
    ...
    else
    {
        if (componentName != NULL)
        {
            ...
            if (strncmp((const char*)componentName, g_thermostatComponent11Name, g_thermostatComponent11Size) == 0)
            {
                result = PnP_ThermostatComponent_ProcessCommand(g_thermostatHandle11, pnpCommandName, rootValue, response, responseSize);
            }
            else
            {
...
static void PnP_TempControlComponent_ApplicationPropertyCallback(const char* componentName, const char* propertyName, JSON_Value* propertyValue, int version, void* userContextCallback)
{
    ...
    else if (strcmp(componentName, g_thermostatComponent11Name) == 0)
    {
        PnP_ThermostatComponent_ProcessPropertyUpdate(g_thermostatHandle11, deviceClient, propertyName, propertyValue, version);
    }
    else
    {
...
static IOTHUB_DEVICE_CLIENT_LL_HANDLE CreateDeviceClientAndAllocateComponents(void)
{
    ...
    else if ((g_thermostatHandle1 = PnP_TelemetriesComponent_CreateHandle(g_thermostatComponent1Name)) == NULL)
    {
        LogError("Unable to create component handle for %s", g_thermostatComponent1Name);
        result = false;
    }
    else if ((g_thermostatHandle3 = PnP_TelemetriesComponent_CreateHandle(g_thermostatComponent3Name)) == NULL)
    {
        LogError("Unable to create component handle for %s", g_thermostatComponent3Name);
        result = false;
    }
    else if ((g_thermostatHandle4 = PnP_TelemetriesComponent_CreateHandle(g_thermostatComponent4Name)) == NULL)
    {
        LogError("Unable to create component handle for %s", g_thermostatComponent4Name);
        result = false;
    }
    else if ((g_thermostatHandle5 = PnP_TelemetriesComponent_CreateHandle(g_thermostatComponent5Name)) == NULL)
    {
        LogError("Unable to create component handle for %s", g_thermostatComponent5Name);
        result = false;
    }
    else if ((g_thermostatHandle6 = PnP_TelemetriesComponent_CreateHandle(g_thermostatComponent6Name)) == NULL)
    {
        LogError("Unable to create component handle for %s", g_thermostatComponent6Name);
        result = false;
    }
    else if ((g_thermostatHandle7 = PnP_TelemetriesComponent_CreateHandle(g_thermostatComponent7Name)) == NULL)
    {
        LogError("Unable to create component handle for %s", g_thermostatComponent7Name);
        result = false;
    }
    else if ((g_thermostatHandle8 = PnP_TelemetriesComponent_CreateHandle(g_thermostatComponent8Name)) == NULL)
    {
        LogError("Unable to create component handle for %s", g_thermostatComponent8Name);
        result = false;
    }
    else if ((g_thermostatHandle9 = PnP_TelemetriesComponent_CreateHandle(g_thermostatComponent9Name)) == NULL)
    {
        LogError("Unable to create component handle for %s", g_thermostatComponent9Name);
        result = false;
    }
    else if ((g_thermostatHandle10 = PnP_TelemetriesComponent_CreateHandle(g_thermostatComponent10Name)) == NULL)
    {
        LogError("Unable to create component handle for %s", g_thermostatComponent10Name);
        result = false;
    }
    else if ((g_thermostatHandle11 = PnP_ThermostatComponent_CreateHandle(g_thermostatComponent11Name)) == NULL)
    {
        LogError("Unable to create component handle for %s", g_thermostatComponent11Name);
        result = false;
    }
    else
    {
    ...
    if (result == false)
    {
        PnP_TelemetriesComponent_Destroy(g_thermostatHandle11);
        PnP_TelemetriesComponent_Destroy(g_thermostatHandle10);
        PnP_TelemetriesComponent_Destroy(g_thermostatHandle9);
        PnP_TelemetriesComponent_Destroy(g_thermostatHandle8);
        PnP_TelemetriesComponent_Destroy(g_thermostatHandle7);
        PnP_TelemetriesComponent_Destroy(g_thermostatHandle6);
        PnP_TelemetriesComponent_Destroy(g_thermostatHandle5);
        PnP_TelemetriesComponent_Destroy(g_thermostatHandle4);
        PnP_TelemetriesComponent_Destroy(g_thermostatHandle3);
...
int pnp_temperature_controller(void)
{
    ...
    else
    {
        ...
        PnP_TempControlComponent_Report_MaxTempSinceLastReboot_Property(g_thermostatHandle11, deviceClient);
        ...
        while (true)
        {
            ...
            if ((numberOfIterations % g_sendTelemetryPollInterval) == 0)
            {
                ...
                PnP_TelemetriesComponent_SendTelemetry(g_thermostatHandle1, deviceClient);
                PnP_TelemetriesComponent_SendTelemetry(g_thermostatHandle3, deviceClient);
                PnP_TelemetriesComponent_SendTelemetry(g_thermostatHandle4, deviceClient);
                PnP_TelemetriesComponent_SendTelemetry(g_thermostatHandle5, deviceClient);
                PnP_TelemetriesComponent_SendTelemetry(g_thermostatHandle6, deviceClient);
                PnP_TelemetriesComponent_SendTelemetry(g_thermostatHandle7, deviceClient);
                PnP_TelemetriesComponent_SendTelemetry(g_thermostatHandle8, deviceClient);
                PnP_TelemetriesComponent_SendTelemetry(g_thermostatHandle9, deviceClient);
                PnP_TelemetriesComponent_SendTelemetry(g_thermostatHandle10, deviceClient);
                PnP_ThermostatComponent_SendTelemetry(g_thermostatHandle11, deviceClient);
            }
